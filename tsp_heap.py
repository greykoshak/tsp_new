...
class HeapifyTSP:
    """ Создание кучи-min: элементом кучи является кортеж (number, obj) """
    ...
    def del_element(self):
        ...
        while fl_moved and (p * 2 + 1) < n:
            ...
            if (right < n) and self.heap[right] < self.heap[left]:
                ptr_min = right
            if self.heap[p] > self.heap[ptr_min]:
                self.heap[p], self.heap[ptr_min] = self.heap[ptr_min], self.heap[p]
                p = ptr_min
                fl_moved = True
